...
if (!class_exists('ALOptimizer')) :
	...
	class ALOptimizer {
		...
		public function removeCommentsPreserveScripts($buffer) {
			...
			$temp_buffer = ALHelper::safePregReplace('/<!--(.*)-->/Uis', '', $temp_buffer);
		...
		public function optimizeBuffer($buffer) {
			...
			if (is_array($template_config) && !empty($template_config)) {
				...
				if (array_key_exists('used_css', $template_config) && ($url_matched || $apply_used_css_on_similar_posts)) {
					...
					if (array_key_exists('image_replace_rules', $template_config)) {
						$buffer = $this->handleImagesInInlineStyle($buffer, $template_config['image_replace_rules']);
					}
				} else {
					if (array_key_exists('critical_css', $template_config) && array_key_exists("stylesheet", $template_config)) {
						...
						if (array_key_exists('delay_stylesheets', $template_config) && $template_config['delay_stylesheets']) {
							if (array_key_exists('delay_only_on_exact_match', $template_config) && $template_config['delay_only_on_exact_match']) {
								if (empty($result['remaining_styles'])) {
									$buffer = $this->modifyInlineStyles($buffer);
									$buffer = $this->modifyExternalStylesheets($buffer, $template_config['stylesheet'], $head_tag);
								}
							} else {
								$buffer = $this->modifyInlineStyles($buffer);
								$buffer = $this->modifyExternalStylesheets($buffer, $template_config['stylesheet'], $head_tag);
		...
		public function modifyExternalStylesheets($buffer, $css_config, $head_tag) {
			if (!isset($this->config['extract_css_regex'])) {
				return $buffer;
			}
